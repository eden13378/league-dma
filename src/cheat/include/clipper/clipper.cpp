...
#define NEAR_ZERO(val) (((val) > -TOLERANCE) && ((val) < TOLERANCE))
    ...
    struct LocMinSorter {
        inline bool operator()( const LocalMinimum& locMin1, const LocalMinimum& locMin2 ) const {
    ...
    void PolyNode::AddChild( PolyNode& child ) {
        const unsigned cnt = ( unsigned )Childs.size( );
...
//------------------------------------------------------------------------------
    ...
    Int128 Int128Mul( long64 lhs, long64 rhs ) {
        ...
        const ulong64 int1Lo = ulong64( lhs & 0xFFFFFFFF );

        if ( rhs < 0 ) rhs = -rhs;
        const ulong64 int2Hi = ulong64( rhs ) >> 32;
        const ulong64 int2Lo = ulong64( rhs & 0xFFFFFFFF );
            ...
            if ( ( ip.Y < pt.Y ) != ( ipNext.Y < pt.Y ) ) {
                if ( ip.X >= pt.X ) {
                    if ( ipNext.X > pt.X ) result = 1 - result;
                        ...
                        const double d = ( double )( ip.X - pt.X ) * ( ipNext.Y - pt.Y ) -
            ...
            if ( ( op->Pt.Y < pt.Y ) != ( op->Next->Pt.Y < pt.Y ) ) {
                if ( op->Pt.X >= pt.X ) {
                    if ( op->Next->Pt.X > pt.X ) result = 1 - result;
                        ...
                        const double d = ( double )( op->Pt.X - pt.X ) * ( op->Next->Pt.Y - pt.Y ) -
    ...
    *******************************************************************************/
    ...
    void Clipper::FixupOutPolygon( OutRec& outrec ) const {
    ...
    bool Clipper::JoinPoints( Join* j, OutRec* outRec1, OutRec* outRec2 ) const {
        ...
        if ( isHorizontal && ( j->OffPt == j->OutPt1->Pt ) &&
            ...
            const bool Reverse1 = ( ( op1b->Pt.Y > op1->Pt.Y ) ||
            ...
            const bool Reverse2 = ( ( op2b->Pt.Y > op2->Pt.Y ) ||
    ...
    void ClipperOffset::OffsetPoint( int j, int& k, JoinType jointype ) {
        ...
        if ( m_sinA * m_delta < 0 ) {
            ...
            switch ( jointype ) {
            case jtMiter: {
                const double r = 1 + ( m_normals[ j ].X * m_normals[ k ].X +
    ...
    void ClipperOffset::DoSquare( int j, int k ) {
        const double dx = std::tan( std::atan2( m_sinA,
    ...
    void ClipperOffset::DoRound( int j, int k ) {
        const double a = std::atan2( m_sinA,
