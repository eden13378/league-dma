...
public:
	...
	**/
	...
	{
		uintptr_t ntos_shutdown = DMA.GetExportTableAddress("NtShutdownSystem", "csrss.exe", "ntoskrnl.exe");
		...
		*reinterpret_cast<uint64_t*>(jmp_bytes + 6) = function;
		...
		if (!DMA.Read(ntos_shutdown, (PBYTE)orig_bytes, sizeof(orig_bytes), 4))
			return buffer;

	
		if (!DMA.Write(ntos_shutdown, jmp_bytes, sizeof(jmp_bytes), 4))
		{
		...
		//Restore function
		if (!DMA.Write(ntos_shutdown, orig_bytes, sizeof(orig_bytes), 4))
			LOG("[!] Failed to write memory at 0x%p\n", ntos_shutdown);
