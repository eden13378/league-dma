...
Memory::ModuleInformation Memory::GetModuleInformation(std::string module_name)
{
	std::vector<std::string> list = { };
	PVMMDLL_MAP_MODULE module_info;
	if (!VMMDLL_Map_GetModuleU(this->vHandle, this->current_process.PID, &module_info, VMMDLL_MODULE_FLAG_NORMAL)) {
	...
	{
		auto module = module_info->pMap[i];
		if(module.uszText == module_name)
			return { module.vaBase, module.cbImageSize };
	}
...
uintptr_t Memory::GetExportTableAddress(std::string import, std::string process, std::string module)
{
	...
	bool result = VMMDLL_Map_GetEATU(DMA.vHandle, DMA.GetPidFromName(process) /*| VMMDLL_PID_PROCESS_WITH_KERNELMEMORY*/, (LPSTR)module.c_str(), &eat_map);
	if (!result)
	{
...
uintptr_t Memory::GetImportTableAddress(std::string import, std::string process, std::string module)
{
	...
	bool result = VMMDLL_Map_GetIATU(DMA.vHandle, DMA.GetPidFromName(process) /*| VMMDLL_PID_PROCESS_WITH_KERNELMEMORY*/, (LPSTR)module.c_str(), &iat_map);
	if (!result)
	{
...
uint64_t Memory::FindSignature(const char* signature, uint64_t range_start, uint64_t range_end, int PID)
{
	...
	std::cout <<" Range: " << std::hex << range_start << " - " << range_end << std::endl;

	uint64_t first_match = 0;
	for (uint64_t i = range_start; i < range_end; i++) {

		auto value = buffer[i - range_start];

		//std::cout << "Value: " << std::hex << (int)value << std::endl;
		//std::cout << "Byte: " << (int)GetByte(pat) << std::endl;
...
{
	if (UsingPermanentHandle) return PermanentHandle;

	VMMDLL_SCATTER_HANDLE ScatterHandle = VMMDLL_Scatter_Initialize(this->vHandle, this->current_process.PID, VMMDLL_FLAG_NO_PREDICTIVE_READ  | VMMDLL_FLAG_NOCACHE | VMMDLL_FLAG_NOPAGING_IO );
	if (!ScatterHandle)
...
VMMDLL_SCATTER_HANDLE Memory::CreateScatterHandle(int pid) {
	...
	VMMDLL_SCATTER_HANDLE ScatterHandle = VMMDLL_Scatter_Initialize(this->vHandle, pid, 
	VMMDLL_FLAG_NOCACHE |
		VMMDLL_FLAG_NO_PREDICTIVE_READ// | VMMDLL_FLAG_NOPAGING_IO
	);
	if (!ScatterHandle)
		LOG("[!] Failed to create scatter handle\n");
	return ScatterHandle;
...
void Memory::CloseScatterHandle(VMMDLL_SCATTER_HANDLE handle) {
	...
	if (UsingPermanentHandle) return;
...
bool Memory::InitializeScatter( ) {

	PermanentHandle = VMMDLL_Scatter_Initialize(this->vHandle, this->current_process.PID, VMMDLL_FLAG_NO_PREDICTIVE_READ | VMMDLL_FLAG_NOCACHE);

	if (!PermanentHandle) LOG("[!] Failed to create scatter handle\n");
	else LOG("[ DMA: Scatter ] Successfully created permanent handle\n");

	UsingPermanentHandle = !!PermanentHandle;

	return !!PermanentHandle;
}

bool Memory::ShutdownScatter() {

	if (UsingPermanentHandle)
	{
		VMMDLL_Scatter_CloseHandle(PermanentHandle);
		PermanentHandle = NULL;
		UsingPermanentHandle = false;
		return true;
	}
...
{
	if (!VMMDLL_Scatter_PrepareEx( UsingPermanentHandle ? PermanentHandle : handle, address, static_cast<DWORD>(size), static_cast<PBYTE>(buffer), nullptr))
	{
...
{
	if (!VMMDLL_Scatter_PrepareWrite(UsingPermanentHandle ? PermanentHandle : handle, address, (PBYTE)buffer, size))
	{
		LOG("[!] Failed to prepare scatter write at 0x%p\n", address);
	}
}

void Memory::ExecuteReadScatter(VMMDLL_SCATTER_HANDLE handle, int pid, bool reset_handle)
{
	if (pid == 0) pid = this->current_process.PID;

	if (!VMMDLL_Scatter_ExecuteRead(UsingPermanentHandle ? PermanentHandle : handle)) {
	...
	if (!reset_handle) return;

	//Clear after using it
	if (!VMMDLL_Scatter_Clear(UsingPermanentHandle ? PermanentHandle : handle, pid, VMMDLL_FLAG_NOCACHE)) {
		LOG("[-] Failed to clear Scatter\n");
	}

}

void Memory::ClearScatterQueue(VMMDLL_SCATTER_HANDLE handle)
{
	if (!VMMDLL_Scatter_Clear(UsingPermanentHandle ? PermanentHandle : handle, this->current_process.PID, VMMDLL_FLAG_NOCACHE)) {
		LOG("[-] Failed to Execute Scatter Clear\n");
	}
}
...
void Memory::ExecuteWriteScatter(VMMDLL_SCATTER_HANDLE handle, int pid)
{
	...
	if (!VMMDLL_Scatter_Execute(UsingPermanentHandle ? PermanentHandle : handle))
	{
		LOG("[-] Failed to Execute Scatter Read\n");
	}

	//Clear after using it
	if (!VMMDLL_Scatter_Clear(UsingPermanentHandle ? PermanentHandle : handle, pid, NULL))
	{
		LOG("[-] Failed to clear Scatter\n");
	}
}
...
bool Memory::IsValidPointer(uint64_t Pointer) {
	return Pointer > 0x00010000 && Pointer < 0x7FFFFFFEFFFF && Pointer != 0;
}
