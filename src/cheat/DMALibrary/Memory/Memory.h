...
public:
	...
	*/
	...
	ModuleInformation GetModuleInformation(std::string module_name);
	...
	*/
	...
	bool Shutdown();
	bool InitializeScatter();
	bool ShutdownScatter();

	VMMDLL_SCATTER_HANDLE PermanentHandle = nullptr;
	bool UsingPermanentHandle = false;
	 ...
	 * \param pid 
	 */
	void ExecuteReadScatter(VMMDLL_SCATTER_HANDLE handle, int pid = 0, bool reset_handle = true);
	void ExecuteWriteScatter(VMMDLL_SCATTER_HANDLE handle, int pid = 0);
	void ClearScatterQueue(VMMDLL_SCATTER_HANDLE handle);

	bool IsValidPointer(uint64_t Pointer);

	CurrentProcessInformation GetCurrentProcess() { return current_process; }
