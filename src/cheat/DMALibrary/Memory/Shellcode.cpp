...
{
	int pid = DMA.GetPidFromName(process_name);
	...
	if (!VMMDLL_ProcessGetInformation(DMA.vHandle, pid, &process_info, &process_info_size))
	{
	...
	DWORD cSections = 0;
	if (!VMMDLL_ProcessGetSectionsU(DMA.vHandle, pid, (LPSTR)module.c_str(), NULL, 0, &cSections) || !cSections)
	{
	...
	if (!pSections || !VMMDLL_ProcessGetSectionsU(DMA.vHandle, pid, (LPSTR)module.c_str(), pSections, cSections, &cSections) || !cSections)
	{
	...
	for (int i = 0; i < cSections; i++)
	{
		...
		{
			codecave = VMMDLL_ProcessGetModuleBaseU(DMA.vHandle, pid, (LPSTR)module.c_str()) + ((pSections[i].VirtualAddress + pSections[i].Misc.VirtualSize)) + 0x10;
			if (!codecave)
				break;
		}
	}
	...
	if (!DMA.Read(codecave, buffer.get(), function_size, pid))
	{
...
std::vector<uint64_t> c_shellcode::find_all_codecave(size_t function_size, std::string process_name)
{
	...
	std::vector<std::string> module_list = DMA.GetModuleList(process_name);
...
bool c_shellcode::call_function(void* hook, void* function, std::string process_name)
{
	...
	*reinterpret_cast<uint64_t*>(jmp_bytes + 6) = (uintptr_t)hook;
	...
	if (!DMA.Read((uintptr_t)function, orig_bytes.get(), sizeof(jmp_bytes), pid))
		return 0;

	if (!VMMDLL_MemWrite(DMA.vHandle, pid, (uintptr_t)function, jmp_bytes, sizeof(jmp_bytes)))
	{
	...
	//Restore function
	if (!VMMDLL_MemWrite(DMA.vHandle, pid, (uintptr_t)function, orig_bytes.get(), sizeof(jmp_bytes)))
	{
