...
namespace li
{
    namespace detail
    {
            ...
            const win::UNICODE_STRING_T& str ) noexcept
        {
            ...
            auto value = hash_t::offset;
            for ( ; first != last; ++first ) value = hash_t::single ( value, static_cast< char > ( *first ) );
            ...
            const char* str ) noexcept
        {
            ...
            for ( ; *str != '.'; ++str ) module_and_function.first = hash_t::single ( module_and_function.first, *str );

            ++str;

            for ( ; *str; ++str ) module_and_function.second = hash_t::single ( module_and_function.second, *str );
...
#error Unsupported platform. Open an issue and I'll probably add support.
        ...
        public:
            ...
            {
                auto& cached = _cache( );
                if ( !cached ) cached = Derived::template get< void*, Enum >( );
        ...
        struct lazy_module : lazy_base< lazy_module< Hash > >
        {
            ...
            LAZY_IMPORTER_FORCEINLINE static T get ( ) noexcept
            {
                ...
                do
                {
                    if ( hash ( e.value->BaseDllName ) == Hash ) return ( T )e.value->DllBase;
                }
                while ( e.next( ) );
                return { };
            }
        ...
        struct lazy_function : lazy_base< lazy_function< Hash, T >, T >
        {
            ...
            return forwarded<F, Enum>();
                ...
                do
                {
                    ...
                    if ( exports )
                    {
                        ...
                        while ( export_index-- )
                            if ( hash ( exports.name ( export_index ) ) == Hash ) return ( F )exports.address ( export_index );
                    }
                }
                while ( e.next( ) );
                return { };
            ...
            LAZY_IMPORTER_FORCEINLINE static F forwarded ( ) noexcept
            {
                ...
                do
                {
                    win::UNICODE_STRING_T name = e.value->BaseDllName;
                        ...
                        if ( exports )
                        {
                            ...
                            while ( export_index-- )
                                if ( hash ( exports.name ( export_index ) ) == function_hash )
                                {
                                    ...
                                    {
                                        const auto hashes = hash_forwarded (
            ...
            LAZY_IMPORTER_FORCEINLINE static F forwarded_cached ( ) noexcept
            {
                ...
                if ( !value ) value = forwarded< void*, Enum >( );
                return ( F )value;
            }
            ...
            LAZY_IMPORTER_FORCEINLINE static F in ( Module m ) noexcept
            {
                ...
                if ( IsSafe && !exports ) return { };

                for ( unsigned long i{ };; ++i )
                {
                    if ( IsSafe && i == exports.size( ) ) break;

                    if ( hash ( exports.name ( i ) ) == Hash ) return ( F )exports.address ( i );
                }
                return { };
            }
            ...
            {
                auto& value = base_type::_cache( );
                if ( !value ) value = in< void*, IsSafe > ( m );
                return ( F )value;
            }
